    ...
    static class Cfg
    {
        public static class Create
        {
            ...
            private static void SongList(Category category, int index)
            {
                ...
                {
                    content.Add(string.Format("echo t{2}: {0}: {1}", sound.Id.ToString("0000"), sound.Name, count));
                    SongLoad(sound);
                    content.Add(string.Format("alias t{0} \"exec radio_category_soundload{1}.cfg\"", count++, sound.Id.ToString("0000")));
                }
            ...
            private static void UnbindAll(int count)
            {
                //List<string> content = new List<string>();

                //for (int i = 0; i < 100; i++)
                //{
                //    content.Add("unbind bs_play bs_play_on");
                //}
                ...
                //File.WriteAllLines(ProgramSettings.PathCsgo + "\\csgo\\cfg\\radio_unbindall.cfg", content);
            }
        }
